...
const todos = createSlice({
  ...
  reducers: {
    //reducers is an object where each property is a method to update the store

    //first method:
    addTodo: (store, action) => {
      //Since we need to pass data about what todo, we need two arguments in this recuder - current value of store and action)
    ...
    deleteTodo: (store, action) => {
      const decreasedItems = store.items.filter(
        (item) => item.id !== action.payload //if id equals to action.payload it's not included in this array
      ); 
